...
var getRange = function getRange(range) {
  if (range.includes('-') === false) console.log('Invalid IP Range, please try:\nnetworker [options] 192.168.1.1-254');
  if (!range) console.log('IP Range cannot be null, please try:\nnetworker [options] 192.168.1.1-254');
  var array = [];
  var network = range.split('-')[0];
  var max = range.split('-')[1];
  if (max > 255) console.log('Invalid IP Range, please try:\nnetworker [options] 192.168.1.1-254');
